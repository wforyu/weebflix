...
- **Resolver client chain (`YouTubeResolver.resolve`):** `ANDROID_VR` (1.65.10, key `AIzaSyB9VGVgUmYc0HeBp5dHnjg1WxNb0qk2X3k`, **wajib footprint VR asli**: UA `com.google.android.apps.youtube.vr.oculus/...` + `osName=Android`/`osVersion=12L`/`deviceModel="Quest 3"`/`deviceMake="Oculus"` — UA plain app yang mismatch adalah pemicu utama "not a bot"; clientVersion wajib ≤1.65 agar tetap dapat format DASH non-SABR) → `ANDROID` (21.26.364, Pixel 5, key `AIzaSyA8eiZmM1FaDVjRy-df2KTyQ_MzhuOLXZ0`) → `ANDROID_MUSIC` (6.27.51, Pixel 7, key `AIzaSyAOghZGza2MQSZkYuz4VlJ4v5wZ7Y4W4sQ`) → `IOS` (19.43.2, iPhone16,2, key `AIzaSyB-63vPrnThHnHxe9cQ9QZQN9QZ9QZQZQ` — bukan lagi pakai WEB key) → `TVHTML5` (Cobalt, key ANDROID) → `MWEB` (iPad Safari) → `WEB_EMBEDDED_PLAYER` (signatureCipher via `YouTubeCipher`). 2.5s sleep antara client; HTTP 400 = flagged → stop. `ClientContext` kini punya `deviceMake` opsional.
...
- **Anti-break strategy (YouTube updates signatures often):**
  ...
  2. **Multi-client fallback chain** — `android_vr` → `android` → `android_music` → `ios` → `tvhtml5` → `mweb` → `web_embedded` (see chain above; `ensureVisitor()` covers the LOGIN_REQUIRED gate, `resetVisitor()` + single ANDROID_VR retry covers a stale/flagged visitor).
...
- **⭐ OAuth login infra — DONE (2026-08-04, code + layout verified on-device; end-to-end butuh client ID asli):** seluruh infrastruktur login terpasang, tinggal user buat Google Cloud OAuth client + isi Client ID:
  ...
  - **Injeksi auth di `YouTubeResolver.fetchPlayer`:** bila `YouTubeAuthManager.getAccessToken()` non-null → header `Authorization: Bearer <token>` + `X-Goog-AuthUser: 0` pada `youtubei/v1/player` (chain client ANDROID_VR→…tetap jalan; visitorData juga dikirim). Request ber-login tidak kena `LOGIN_REQUIRED` bot-gate
  - **⚠ Auth = 2-pass fallback (fixed 2026-08-05):** YouTube saat ini **memblokir SEMUA request innertube ber-OAuth** — `youtubei/v1/player` (dan `browse` dll) dengan `Authorization: Bearer <token>` + scope `youtube` penuh → **HTTP 400 `INVALID_ARGUMENT`** di SEMUA client (ANDROID_VR/ANDROID_MUSIC/ANDROID/WEB = 400, IOS = 404), dengan/ tanpa `visitorData`, bahkan untuk video publik normal (`dQw4w9WgXcQ`). Ini bug sisi Google, bukan app: **open issue LuanRT/YouTube.js #916 (+ #803)** sejak 2025-03, belum ada fix — semua client pihak ketiga (termasuk YouTube.js sendiri) kena. Login tidak membantu playback saat ini. Fix di `YouTubeResolver.resolve()`: **pass 1 coba dengan auth (best-effort) → pass 2 anonymous** — kalau pass auth gagal (HTTP 400/401/403 → `flagged` → break), retry seluruh chain TANPA header auth. Konsekuensi: login TIDAK pernah merusak playback, dan `Ihtxx2s6RUE` (LAPOR PAK!) yang dulu `LOGIN_REQUIRED` sekarang **status=OK + streamingData penuh tanpa login** (verified 2026-08-05, fresh visitor + ANDROID_VR) — gate lama sudah hilang. Keep auth-first: kalau Google suatu saat membuka lagi OAuth innertube, gated videos langsung ke-bypass tanpa perubahan kode
...
- **Pinch-to-zoom video (2026-08-14, semua provider):** `ScaleGestureDetector` di `gestureOverlay` + `webViewPlayerControls` — pinch 2 jari (hanya saat fullscreen, `isSystemBarsHidden`) zoom video **1x–4x** (`maxVideoZoom`), pivot ikut fokus pinch (`applyVideoZoom`, konversi offset saat surface ≠ overlay). Berlaku ke surface visible: `playerView` (ExoPlayer) atau `webView` (WebView playback). `resetVideoZoom()` dipanggil di `playEpisodePageViaWebView`/`playVideoViaHtml5WebView`/`showExoPlayerUi`/toggle fullscreen. Gesture 2-jari tidak bocor ke brightness/volume/seek — pinch detector di-feed duluan, `pinchActive` guard memblokir `gestureDetector`.
...
## TODO / Next Session
- **YouTube mini player — DONE (2026-08-08, build + install verified, sisa on-device interaktif):** collapse via pull-down cepat + home feed + pull-to-refresh + volume-gesture fix — lihat bullet "Mini player" di bagian Achieved. Sisa verifikasi manual: tarik lambat TIDAK lagi menurunkan volume, spinner refresh muncul saat tarik feed.
