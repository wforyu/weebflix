...
- **Version:** `versionCode=118`, `versionName=2.0.18-beta` (in `app/build.gradle.kts`)
...
- Key methods: `getLatestEpisodes(page)` (homepage latest, scoped to `div.releases.latesthome` to avoid Popular Today duplicates), `getOngoingAnime(page)` (`/ongoing/page/{N}/`), `getPopularAnime(page)` (`/completed/page/{N}/`), `getAllAnime(page)` (**⚠ path berubah: `/anime/` + `/anime/page/{N}/` — `/seri/` kini 301 → home, rusak sejak migrasi ke `anichin.moe` 2026-09**), `searchAnime(query)`, `getAnimeDetail(url)`, `getEpisodeServers(url)` (base64-decoded `<select class="mirror">`), `getEpisodeNavigation(url)` (`a[rel=prev/next]`, href relatif dinormalisasi ke absolute)
- **Detail resolution:** `getAnimeDetail()` on an *episode* URL (e.g. from a Latest Episode card) resolves to the series page via breadcrumb; episode pages have NO episode list (`div.eplister` only exists on series pages). **⚠ Breadcrumb berubah (2026-09):** kini `<div class="ts-breadcrumb bixbox">` + `<a itemprop="item" href="/{slug}/">` — TANPA `/seri/`, tanpa `ol li`. Fallback scraper: prefer link `/seri/`/`/anime/` di breadcrumb → else `a[itemprop='item']` pertama yang bukan `/`, lalu absolut-kan href relatif dengan `baseUrl`
- **Server resolution (⚠ berubah drastis 2026-09):** episode page baru TIDAK lagi pakai `anichin.stream` — server video kini murni iframe-embed di `<select class="mirror">` (9 pilihan): `anichin-player.web.id/index.php?video=` (proxy Dailymotion), `ok.ru/videoembed/`, `rumble.com/embed/`, `anichin.rpmvid.com/#{id}` (SPA), `play.d.tube/?v={uuid}` (D-Tube), `turbovidhls.com/t/{id}` (New Player), `player.abyssplayer.com/{id}` (hydrax/SoTrym), `rubyvidhub.com/embed-*.html`, `morencius.com/embed/{id}` (VidHide). Semua di-return apa adanya oleh `resolveServerVideoUrl()` (`isBrowserPlayableEmbed()`) dan dimainkan di visible WebView via `isWebViewPlayableEmbed()` di `PlayerActivity` (host baru `anichin.rpmvid.com`/`morencius`/`d.tube`/`turbovidhls` ditambahkan 2026-09). Path `anichin.stream` lama **DIBUANG 2026-09** (`anichin.stream` sudah mati, HTTP 000 — branch khusus + referer interceptor + cleanHls `1a-1791.com` dihapus dari `AnichinScraper`/`PlayerActivity`)
...
- **Search:** Real-time search with debounce (500ms) + Search history (SharedPreferences, max 20). **YouTube search = infinite scroll (2026-08-29):** `YouTubeScraper.searchPage()/nextSearchPage(continuation)` (post `search` + continuation chaining via `sectionListRenderer`, tangani `onResponseReceivedCommands` + `onResponseReceivedActions`); `YouTubeSearchAdapter` punya TYPE_VIDEO/TYPE_FOOTER (`submitPage`/`append`/`setLoading`, footer reuse `item_youtube_footer.xml`); `YouTubeSearchActivity` scroll listener (loadMore saat `lastVisible >= total-6`) + `seenIds` dedup + `loadMoreJob` di-cancel di `performSearch`. Terverifikasi 4 halaman continuation 18/18/19/12 video fresh, token 612→580→528→602→434, `ended=false`
...
- **Player:** ExoPlayer, server picker (floating PopupWindow), gestures (brightness/volume/seek — volume akumulasi float kontinu biar smooth, bukan step int), **pinch-to-zoom video 1x–4x** (fullscreen, semua provider, ExoPlayer & WebView), skip opening/outro (smart windows: intro = first `min(120s, 12%)` OR mid-episode `210s–min(330s, 30%)` if episode ≥11min; outro = last `min(120s, 8%)`), auto-play next episode, PiP support + **kotak play/pause PiP jalan via MediaSession** + **custom PiP actions `com.weebflix.app.PIP_PLAY`/`PIP_PAUSE` di-wire ke BroadcastReceiver dinamis** (defensif API 31+, `registerPipActionReceiver()` di onCreate, unregister di onDestroy, `RECEIVER_NOT_EXPORTED` pada API 33+), **audio lanjut saat layar dikunci / app di-background (pola GoTube — `onPause` tidak lagi pause player; `setWakeMode(WAKE_MODE_NETWORK)` + media3 `MediaSession` = kontrol sistem volume panel / quick settings / PiP)** (2026-08-28), **auto-play YouTube lanjut saat background/layar dikunci (2026-08-29):** `onPause` menahan `progressUpdateRunnable`+`autoPlayRunnable` saat YouTube & player masih playing (`keepChain`; provider lain tetap di-cancel — navigate pakai `startActivity` tidak cocok dipicu dari background) biar rantai video berikutnya tidak putus; `onResume` add `removeCallbacks` sebelum `postDelayed` biar tidak ada runnable ganda; **sleep timer (2026-08-29):** gear player → `showYtSettingsMenu()` (Resolusi video + Timer tidur), `btnSleepTimer` ⦿ merah saat aktif, opsi Off/akhir video/15-30-45-60 mnt (+1,5-3 jam utk video panjang), `fireSleepTimeout()` pause + clear `FLAG_KEEP_SCREEN_ON` + turunkan `SCREEN_OFF_TIMEOUT` 5s via WRITE_SETTINGS (prompt sekali) biar layar benar-benar mati, fullscreen toggle, prev/next episode navigation, **hold-seek / tahan-untuk-FF (Telegram/YouTube-style, 2026-08-29):** long-press ~320ms di separuh KANAN layar → video **benar-benar** dipercepat lewat `PlaybackParameters` (bukan seek): mulai x2, **ramp naik 1 level tiap 3 detik tahan** (`holdSeekPosRunnable` — 2x→3x→4x→5x, diskret bukan per-tick biar ga reconfig renderer tiap frame), indikator tengah menunjukkan **durasi besar `pos / total`** (`seekTimeText`, update tiap 250ms) + badge `xN ⏩`; **watchdog buffer** (drain ~0.6s/s karena CDN googlevideo/surrit dsb. cuma sanggup ~1.4–1.8x): saat buffer <6s turun ke 1.5x **sekali** (hysteresis: kembali ke level saat >10s) supaya tahan panjang tidak pernah beku total; track adaptive di-down ke ≤480p saat tahan (`applyHoldLowTrack`, di-restore persis saat lepas); lepas jari = balik ke kecepatan semula tanpa jump (putar lanjut dari posisi hasil FF); arming pakai timer sendiri (bukan GD onLongPress) biar nggak bentrok volume, aktif-hold kebal dari pinch (`onScaleBegin`→false) & jari-2 (`pointerCount>=2` hanya cancel saat !active); YouTube: skip prev/next (`ytPlayHistory` + `ytUpNext`) + gear resolusi + default resolusi maks dari Settings; **daftar rekomendasi fullscreen** (`ytFullscreenPanel`/`ytFullscreenList`, link `Rekomendasi` di `ytActionRow` pakai `sharedVideos` di `YouTubeFeedAdapter`, muncul hanya saat YouTube + fullscreen + controls visible + bukan mini) **mini player** dengan home feed + **search langsung dari feed** (lihat bullet Mini player di Achieved)
- **Settings:** Per-provider domain configuration with chip selector, validation, and reset; YouTube default max resolution (Auto/144→2160); **provider visibility toggle** (hide/show MissAV — `provider_enabled_missav`, switch `swMissavEnabled` in Settings → Visibilitas Provider; **default hidden** setelah instal/update, user enable manual); About section shows app version (`2.0.16-beta`) + `GIT_COMMIT` + `BUILD_DATE` from BuildConfig
...
| Anichin | Premium (REMOVED 2026-09) | `anichin.stream` dead (HTTP 000) → branch dihapus | — diganti iframe mirror (`select.mirror`) via WebView |
| Anichin | Dailymotion [ADS] | `anichin-player.web.id/index.php?video={id}` → iframe `geo.dailymotion.com` → metadata `dailymotion.com/player/metadata/video/{id}` → `qualities.auto[0].url` (signed m3u8) | **ExoPlayer** — private video resolved via `AnichinScraper.resolveDailymotion()` (2026-09, verified live `xbbg6hq`/`k6nMt3ZULCkR1mKjuWi`: master/child/init/segment all 200 with `Referer: dailymotion.com`, no cookies needed; fMP4 `ftypisom` init + `.m4s` segments). Token m3u8 ~10 min → refetch per resolve. Referer/Origin + cleanHls via interceptor/`defaultRequestProperties` (`dailymotion.com`/`dmcdn.net`) |
| Anichin | OK.ru | `ok.ru/videoembed/{id}` | WebView (host 403 to direct OkHttp) |
...
- **anichin.stream (REMOVED 2026-09)**: legacy JWPlayer page (`anichin.stream/?id={id}`) is **dead** (HTTP 000). Branch dihapus dari `AnichinScraper.resolveServerVideoUrl` + `PlayerActivity` (referer interceptor, `defaultRequestProperties`, cleanHls `1a-1791.com`, `isTvExoPlayerPreferred`) — episode Anichin baru murni iframe mirror. Catatan historis: packed JS unwrap ke `file:"/hls/{id}.m3u8"` di `1a-1791.com`; `unpackPackedJs()` di AnichinScraper/SamehadakuScraper diperbaiki dengan algoritma packer `e()` (`token(i) = (i<base? "" : token(i/base)) + (i%base>35 ? chr(i%base+29) : (i%base).toString(36))`).
...
- Buffer (conditional in `initExoPlayerRemote`): **clean HLS** (`/hls2/`, `.urlset/`, `dramiyos-cdn.com`, `acek-cdn.com`, `minochinos`, `surrit.com`) → generous `30s/120s/15s/10s` (smooth, no 429 risk); **everything else** (turboviplay etc.) → tight `5s/20s/3s/2s` (fewer concurrent segment requests to avoid 429)
- Cache: `SimpleCache` with 250MB limit, turboviplay URLs bypass cache (unique cache key) to prevent stale re-fetches
- OkHttp: Adds `Referer`/`Origin` headers for `googlevideo.com`, `abysscdn.com`/`hydrax`/`drakor.bid`, `turboviplay.com` (Referer = `turbovidhls.com`), `cloudflarestorage.com` (Referer = `filedon.co/`), `drakorkita.stream`/`drakorP2pHosts` (Referer = `drakorkita.stream`), `surrit.com` (Referer = `missav.ws`)
...
- Video resolution: episode pages baru = murni iframe-embed `<select class="mirror">` (tanpa `anichin.stream`, lihat blok Providers di atas). `resolveServerVideoUrl()` mengembalikan embed apa adanya via `isBrowserPlayableEmbed()` (host baru: `anichin.rpmvid.com`/`morencius`/`d.tube`/`turbovidhls`); path lawas `anichin.stream` (JWPlayer HLS unpacked eval JS → m3u8) **dibuang 2026-09** (server mati, HTTP 000)
- **Episode URL → series:** `getAnimeDetail()` on an episode URL (no `div.eplister` found) re-fetches the series page dari breadcrumb — **⚠ struktur baru (2026-09):** `<div class="ts-breadcrumb bixbox">` + `<a itemprop="item" href="/{slug}/">` (TANPA `/seri/`, tanpa `ol li`); fallback prefer `a[href*='/seri/']`/`a[href*='/anime/']` → else `a[itemprop='item']` pertama non-`/`, lalu absolut-kan href relatif
- **Old-post embeds:** `resolveServerVideoUrl()` returns Dailymotion/Mega/archive.org/OK.ru/Rumble/`anichin-player.web.id`/rubyvidhub/`anichin.rpmvid.com`/`morencius`/`d.tube`/`turbovidhls` embed URLs unchanged (`isBrowserPlayableEmbed()`); `PlayerActivity.isWebViewPlayableEmbed()` routes them to visible-WebView playback (`playEpisodePageViaWebView(skipInjections=true)`) instead of hidden-WebView interception
...
- **Verified selectors (2026-07); +Anichin/Otakudesu/MissAV 2026-08-28:** Samehadaku latest = `ul > li[itemscope] > h2.entry-title a`, Samehadaku grids (ongoing `/daftar-anime-2/`, popular `?order=popular`) = `.animposx a`; DrakorKita = `.bungkus` (url `a[href*='detail/']`, title = first text node of `.titit`, img `img.poster`, `/all?media_type=movie|tv`); OppaDrama = `article.bs .bsx` (+ `user_is_human` cookie via `?verify_human=1`); Anichin = `div.listupd article.bs > .bsx > a` (latest scoped ke `div.releases.latesthome`; paths `/ongoing/` `/completed/` `/anime/`, all_path `/anime/` sejak 2026-09, href relatif di-absolut-kan via `urljoin`); Otakudesu = `div.venz > ul > li > div.detpost` (latest = `.epz` regex `^Episode \d+`, ongoing `/ongoing-anime/` single page, popular `/complete-anime/`); MissAV = `.thumbnail.group` (`a[href*='/id/']` + `.my-2 a`, paths `/id/release` + `?sort=published_at|weekly_views`, `verify=False` — **belum tembus Cloudflare "Just a moment" dari IP dev/GHA, guard skip JSON**). Semua home fragment kini pakai fallback GitHubDataFetcher (mirror pola Samehadaku: memory → disk → GitHub → live)
...
| Anichin latest episode card opens detail with no episode list | Episode URLs have NO `div.eplister` (only series pages do). `getAnimeDetail()` detects empty episode list and re-fetches the series page via breadcrumb — **⚠ berubah 2026-09:** `<div class="ts-breadcrumb bixbox">` + `a[itemprop='item']` (TANPA `/seri/`, tanpa `ol li`); fallback prefer `/seri/`/`/anime/` → else `itemprop='item'` pertama non-`/`, absolut-kan relatif |
...
| Video YouTube `LOGIN_REQUIRED` di semua client padahal user sudah login OAuth + subscribed | Aksaranya **request innertube player tetap anonim** karena pass auth di-gate cookie: `fetchYouTubeCookies()` tak pernah menangkap SID/HSID/SAPISID (`No auth cookies captured`, step 2 gagal `Failed to connect to localhost/127.0.0.1:8080` karena follow-redirect ke loopback mati) → `getSapisid()` null → pass auth di-skip → Bearer cuma dikirim ke client yang menolak (ANDROID/IOS → HTTP 400), yang menerima OAuth (TVHTML5) tidak pernah dikirimi → semua client `LOGIN_REQUIRED "Login untuk mengonfirmasi bahwa Anda bukan bot"`. Fix 2 lapis (`YouTubeResolver.kt`, 2026-08-28): (1) **OAuth-TV pass** — ketika `getAccessToken()!=null`, coba **2× TVHTML5 dengan `Authorization: Bearer <accessToken>`** PALING AWAL sebelum cookie pass & chain anonim — per **YouTube.js docs** OAuth hanya diterima oleh TV innertube client. (2) **UA fix (2026-08-28, verifikasi device 2026-08-29)** — `TVHTML5` mula-mula return `UNPLAYABLE "Halaman perlu dimuat ulang"` di kedua varian walau Bearer dikirim, karena YouTube **mulai menolak semua User-Agent keluarganya Cobalt pada endpoint player TVHTML5 sejak ±2026-08-18** (UNPLAYABLE terlepas dari clientVersion/signatureTimestamp/params). Fix = pakai **PlayStation 4 Safari UA** `Mozilla/5.0 (PlayStation; PlayStation 4/12.00) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Safari/605.1.15` + clientVersion `7.20250319.10.00` + `params="2AMB"` (konfigurasi persis yang di-ship lavalink-devs/youtube-source Tv.java, via PR #233 yang merujuk isu #226). Identitas PS4 masih diterima → `status=OK`. Cookie bootstrap step 2 masih rusak tapi bukan blocker playback (OAuth-TV tak butuh cookie) |
| Auto-play YouTube saat background crash `IllegalStateException: Session ID must be unique. ID=` (`MediaSession.java:732`) | **media3 1.5.1** menyimpan session di **static map global** (`SESSION_ID_TO_SESSION_MAP`) dengan default session ID = `""`. Dua inisialisasi player nyaris bersamaan (mis. rantai auto-play saat layar dikunci — inisialisasi video berikutnya dimulai sebelum session lama dilepas di `navigateToNextEpisode`) membangun dua `MediaSession` ber-`""` → collide. Fix (`PlayerActivity.kt` `rebindMediaSession`, 2026-08-29): `MediaSession.Builder(this, player).setId(java.util.UUID.randomUUID().toString()).build()` biar tiap build dapat ID unik. Terverifikasi on-device (2026-08-29): video di pos 200/212s → background → countdown 10s → video berikutnya di-resolve + `rebindMediaSession built ok` (setelahnya tanpa error `must be unique`) — rantai lanjut mulus di background |
...
- **Resolver client chain (`YouTubeResolver.resolve`):** **OAuth-TV pass (2026-08-28, dijalankan PALING AWAL saat `getAccessToken()!=null`)** — 2× TVHTML5 (`7.20250319.10.00` = versi lavalink Tv.java + `7.20260707.07.00`, **keduanya pakai PlayStation 4 Safari UA** + `params="2AMB"`, key ANDROID, Bearer-only, no PO token) karena **YouTube hanya menerima OAuth di TV innertube client** → lalu (kalau cookie ada) pass cookie-based → lalu **anonymous chain:** `VISIONOS` (1.02, key WEB, **primary — direct URLs, no PO token**) → `ANDROID` (21.26.364, sdk_gphone64_x86_64, key ANDROID, PO token) → `ANDROID` (19.29.37, sdk_gphone64_x86_64, key ANDROID, PO token) → `TVHTML5` (**PlayStation 4 Safari** UA, key ANDROID, `params="2AMB"`) → `MWEB` (iPhone Safari, key WEB, PO token) → `IOS` (19.43.2, iPhone16,2, key IOS, PO token) → `WEB_EMBEDDED` (signatureCipher via `YouTubeCipher`) → `ANDROID_MUSIC` (6.27.51, Pixel 7, PO token) → `ANDROID_VR` (1.65.10, **dead since 2026-08-17**, CDN 403s, `skipStreamingPot=true`, last resort only). 2.5s sleep antara client; HTTP 400 = dead client (not flagged). `ClientContext` punya `deviceMake`, `skipStreamingPot`, `usePoToken` opsional. ⚠ **UA penting untuk TVHTML5 sejak 2026-08-18:** YouTube tolak semua UA keluarga Cobalt di endpoint player TVHTML5 (`UNPLAYABLE "The page needs to be reloaded"`, terlepas dari clientVersion/signatureTimestamp/params) — fix lavalink `youtube-source#233` (2026-08-19). JANGAN kembalikan Cobalt UA ke client mana pun (OAuth maupun anonymous TV).
...
- **⭐ OAuth login infra — DONE (2026-08-04, code + layout verified on-device; end-to-end butuh client ID asli):** seluruh infrastruktur login terpasang, tinggal user buat Google Cloud OAuth client + isi Client ID:
  ...
  - **✅ OAuth-TV pass fix — DONE code (2026-08-28), ✅ UA fix (2026-08-28, verifikasi device 2026-08-29):** per **YouTube.js docs (2026)** *"OAuth2 authentication now only works with the TV Innertube client"* — sejak pertengahan 2026 request **anonim** ke `youtubei/v1/player` dari IP flagged (ISP KISS FAMILY) mulai `LOGIN_REQUIRED` di WEB/ANDROID/IOS/WEB_EMBEDDED walau pakai fresh visitor + PO token (video `9c2wPhSYTGA`, logcat 2026-08-28: semua client LOGIN_REQUIRED). Akar masalah di app: pass auth di-gate — hanya jalan kalau cookie bootstrap sukses (`getYouTubeCookies()`/`getSapisid()` non-null), padahal cookie bootstrap SULIT null (`No auth cookies captured`, step 2 gagal `Failed to connect to localhost/127.0.0.1:8080` karena follow-redirect ke loopback yang tidak listening & Bearer tidak dipakai Google utk authorize) → jadi walau user login OAuth, request player TETAP anonim → bot-gate. **Fix 1 (`YouTubeResolver.resolve()`):** pass baru **OAuth-TV** dijalankan PALING AWAL kalau `getAccessToken() != null` — coba 2 `ClientContext` TVHTML5 via `fetchPlayer(..., auth=true)`; `fetchPlayer` auth-block fallback ke `Authorization: Bearer <token>` (bukan cookie) → TV client SENDIRI yang menerima OAuth. Gagal → lanjut cookie pass → anonymous. **Fix 2 (`YouTubeResolver.kt`, 2026-08-28):** setelah OAuth-TV tetap `UNPLAYABLE "Halaman perlu dimuat ulang"` di kedua varian Cobalt, ditemukan akar barunya — **YouTube menolak semua UA keluarga Cobalt di endpoint player TVHTML5 sejak ±2026-08-18** (lavalink-devs/youtube-source PR #233 → issue #226, dan yt-dlp #17389: `tv_downgraded` "The page needs to be reloaded"). Fix = **PlayStation 4 Safari UA** `Mozilla/5.0 (PlayStation; PlayStation 4/12.00) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Safari/605.1.15` + clientVersion `7.20250319.10.00` + `params="2AMB"` — identitas PS4 masih diterima → `status=OK`. Diterapkan ke **kedua** `tvOauthClients` DAN client `TVHTML5` anonymous chain (Cobalt kini dead di mana pun). **Sisa cookie bootstrap:** step 2 tetap rusak (`followRedirects(true)` ke localhost mati) tapi bukan blocker untuk playback — OAuth-TV tidak butuh cookie.
...
- **Rekomendasi fullscreen (2026-08-29):** panel `ytFullscreenPanel` + `ytFullscreenList` di dalam `playerArea` — 16:9 thumb list video rekomendasi (sumber `sharedVideos`, feed awal yang belum dikonsumsi) yang tampil saat video diputar **fullscreen di provider YouTube**; dicebar dari buka → geser cepat ke atas (`onFling`) toggle fullscreen, panel muncul bersama controls (gate `controlsVisible + !mini`), link `Rekomendasi` di `ytActionRow` (`btnYtFullscreenFeed`) juga buka/tutup panel; tap item → video pindah (anti-ganda mencegah duplikat feed, anti-sink `currentYtVideoId`), panel tetap terbuka. Layout + strings `yt_fullscreen_feed_title`/`yt_fullscreen_feed_hide`
