...
- **⭐ OAuth login infra — DONE (2026-08-04, code + layout verified on-device; end-to-end butuh client ID asli):** seluruh infrastruktur login terpasang, tinggal user buat Google Cloud OAuth client + isi Client ID:
  - `data/auth/YouTubeAuthManager.kt` — Authorization Code + **PKCE** (S256), state/verifier di-memory, swap code→token di `oauth2.googleapis.com/token` (client_secret opsional — support Web-app **dan** Android client), refresh otomatis bila access token expired (±3600s), simpan `refresh_token`/`access_token`/`expires_at`/`email` di SharedPreferences `weebflix_yt_auth`, fetch email via `tokeninfo`. **NOTE:** refresh token masih plain SharedPreferences (prototype) — upgrade EncryptedSharedPreferences sebelum rilis publik
  - `ui/youtube/YouTubeLoginActivity.kt` — consent screen Google di WebView (UA desktop Chrome untuk hindari blokir WebView); **redirect loopback `http://localhost:8080/callback` di-intercept via `shouldOverrideUrlLoading`** (tanpa HTTP server lokal), extract `code`+`state`, `exchangeCode()`, selesai RESULT_OK/CANCELED. Error/`error=` query → toast + finish
  ...
  - **Verified on-device 2026-08-04 (Redmi Note 8):** build SUCCESS; tombol `ytBtnLogin` muncul di view hierarchy (862,22-1036,132); `YouTubeLoginActivity` tanpa client ID → pesan "OAuth belum dikonfigurasi" tampil; dengan client ID dummy → WebView render halaman Google accounts (screenshot white% 76.1) — pipeline URL + WebView + intercept terbukti jalan. **Blocker user:** pastikan `http://localhost:8080/callback` terdaftar di **Authorized redirect URIs** client (Web application), email user terdaftar sebagai Test User, lalu test end-to-end video `Ihtxx2s6RUE` yang tadinya `LOGIN_REQUIRED`
...
- **Login Google OAuth (phase 2) — INFRA DONE + credential built-in, tinggal test end-to-end** — seluruh kode login sudah terpasang (lihat "OAuth login infra" di atas); blocker tinggal pastikan `http://localhost:8080/callback` di Authorized redirect URIs + email sebagai Test User, lalu test video `Ihtxx2s6RUE` yang tadinya `LOGIN_REQUIRED`. Bonus (belum dikerjakan): history sinkron server-side (`FEhistory`)
...
## TODO / Next Session
- **YouTube OAuth login — INFRA DONE (2026-08-04), test end-to-end butuh redirect + Test User:** seluruh kode login terpasang (`YouTubeAuthManager` + `YouTubeLoginActivity` + injeksi Bearer di `YouTubeResolver` + UI di home/settings + **built-in client ID/secret**). User tinggal: pastikan `http://localhost:8080/callback` ada di Authorized redirect URIs, daftarkan email sebagai Test User, install APK → tab YouTube → Masuk → login akun sendiri → coba video `Ihtxx2s6RUE` (dulu `LOGIN_REQUIRED`). Kalau refresh token tidak keluar → cek consent screen mode Testing + email Test User. Upgrade token store ke EncryptedSharedPreferences sebelum rilis publik.
...
- **Android TV support (pertanyaan user 2026-08-03, belum dikerjakan):** APK Android biasa jalan di Android TV (sideload), tapi perlu penyesuaian:
  - **Phase 1 — "bisa jalan + muncul di launcher" (~1-2 jam):** (1) tambah `<category android.intent.category.LEANBACK_LAUNCHER/>` di intent-filter MainActivity; (2) `android:banner` icon TV 320x180; (3) `uses-feature` `android.hardware.touchscreen required=false` + `android.software.leanback required=false` (wajib kalau publish Play Store TV); satu APK yang sama dipakai untuk HP + TV box
  - **Phase 2 — "nyaman dipakai remote" (~1-2 hari):** (1) layout khusus TV (`layout-land`/`sw600dp`); (2) fokus handling D-pad di semua RecyclerView/chip (app ini banyak ngandelin sentuhan: gesture brightness/volume/skip player, swipe hero carousel, chip scroll); (3) mode player khusus TV — pakai default controls ExoPlayer (D-pad friendly), gesture off; (4) orientation: non-YouTube sudah landscape, tapi provider YouTube di-force portrait → perlu penyesuaian di TV (landscape); (5) WebView player (Mega/abyssplayer/dll) navigasi D-pad di dalam WebView sulit → prefer ExoPlayer; (6) search on-screen keyboard di TV menyulitkan
