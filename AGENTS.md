...
- **YouTube-like UI penuh — DONE (2026-08-06, build verified):** player kini punya action row like/dislike (auth-gated, optimistic toggle) + tombol Subscribe (auth-gated) + section Komentar (via `youtubei/v1/next` commentThreads, infinite scroll) + like count & channel id dari owner renderer (RelatedPage). Related-by-video (`relatedVideos`) & search filter chips sudah fungsional sejak sebelumnya. Sisa: player masih generic (belum like/dislike state sinkron server — endpoint innertube ber-OAuth masih diblokir Google, lihat item ini di bawah), resolusi default global
