...
## YouTube Provider (PLANNED — ad-free, no-login-first, prototype pending)
...
- **Anti-break strategy (YouTube updates signatures often):**
  ...
  2. **Multi-client fallback chain** — try `android_vr` → `tv` → `web_embedded` → `android` (some return simpler signatures / empty n-param).
  3. **Invidious/Piped as safety net** — if decipher fails everywhere → Invidious instance for same video → proxied ad-free stream.
  4. **Community patterns** — mirror rule formats from yt-dlp / NewPipe releases.
  5. **Re-resolve on stale** — YT stream URLs last ~6h → 403 → re-resolve fresh (same pattern as other resolvers).
...
- **Effort estimate:** 3-4x of Anichin provider (decipher + OAuth + new UI). Prototype first.

## TODO / Next Session
- **YouTube provider prototype** — implement `YouTubeScraper` + `youtube://` route in `PlayerActivity` + `YouTubeResolver`/`YouTubeCipher` + `MergingMediaSource`, no login, no full UI (per plan above). Verify ad-free playback on-device before login + UI phase.
