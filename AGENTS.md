...
- **Full token-free flow (verified live on TV ep + movie):**
  ...
  4. Decrypted JSON contains `source` (direct-IP m3u8 `https://185.237.107.x/v4/.../master.m3u8?v=...`, Referer-gated) and **`cfNative`** (`https://drakorkita.stream/v4/pl/{cfDomain}/x68/{hash}/master.{version}.m3u8?k=...&kx=...`, `kx` ≈ now+1h) → **prefer `cfNative`** (stable domain; `k`/`kx` propagate to child playlists automatically).
...
- **Code:** `DrakorKitaScraper.kt` — `fetchDlFileMobJson()` (refactored from `resolveDlFileMob`), `resolveP2pHls(downloadId)` (dlfilemob → hash → fetch hex → `decryptDkP2p()` AES-CBC → JSON → `cfNative` else `source`), `decryptDkP2p(hex)` (javax.crypto AES/CBC/PKCS5). `resolveDownloadServers` **prepends a `"DrakorKita HLS"` server** (`dataType="p2p"`, `videoUrl`=m3u8) at index 0 → auto-selected; dl servers kept as fallback (HLS fails → ExoPlayer error → next server = dl download).
- **`PlayerActivity`:** HLS URL is a `.m3u8` → existing direct-video branch → `initExoPlayer` (no `isDrakorDl` flag needed — fast, no pre-roll). OkHttp interceptor + `initExoPlayerRemote` `defaultRequestProperties` add `Referer`/`Origin: https://drakorkita.stream/` for hosts `drakorkita.stream` and `185.237.107.*` (direct-IP `source`).
