...
- **Account header profil (2026-08-08):** OAuth scope diperluas ke `youtube openid email profile`; `YouTubeAuthManager.fetchUserInfo()` ambil name/picture/email dari `oauth2/v2/userinfo` (cache di token store); `YouTubeHomeFragment` header kini menampilkan avatar (`ytAccountAvatar`, Glide) + nama (`ytAccountName` via `displayName()`, fallback prefix email) saat login, tombol "Masuk" merah saat logout. Old refresh-token dari login pra-scope → userinfo HTTP 403 → header fallback ke prefix email (aman).
