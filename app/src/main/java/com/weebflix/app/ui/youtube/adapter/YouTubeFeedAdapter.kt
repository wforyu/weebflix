...
class YouTubeFeedAdapter(
    ...
    override fun getItemViewType(position: Int): Int =
        if (position < videos.size) TYPE_VIDEO else TYPE_FOOTER
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == TYPE_VIDEO) {
            ...
            FooterVH(LayoutInflater.from(parent.context).inflate(R.layout.item_youtube_footer, parent, false))
        }
    }

    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            is FeedVH -> videos[position].let { holder.bind(it) }
